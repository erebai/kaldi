...
#set the path to the training, development, and evaluation folders
  data_train="speech/train" # directory which contains the training dataset
...
numGaussSGMM=9000
#set DNN parameters
DNN_technique="autoencoder" #Define the neural net technique: dnn|dbn|autoencoder
# get more information about how to set DNN nnet2 parameters: http://kaldi-asr.org/doc/dnn2.html


############### dnn configuration ################
if [ $DNN_technique == "dnn" ]; then
use_gpu=false
train_stage=-100
...
############### dbn configuration ################
if [ $DNN_technique == "dbn" ]; then
nnet_dir=$exp_dir/nnet
data_fmllr=$nnet_dir/data_fmllr
depth=5
hid_dim=64
rbm_iter=1
learn_rate=0.008
...
if [ $DNN_technique == "autoencoder" ]; then
nnet_dir=$exp_dir/autoencoder
data_fmllr=$nnet_dir/data_fmllr
hid_layers=4
hid_dim=64
learn_rate=0.008
...
echo ============================================================================
	...
	if [ ! -d data/train_$sub_train_data ]; then
