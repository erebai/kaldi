...
if [ $# -ne 4 ]; then
  echo "Usage: $0 [options] <graph-dir> <data-dir> <nnet-models> <decode-dir>"
  echo " e.g.: $0 --transform-dir exp/tri3b/decode_dev93_tgpr \\"
  echo "      exp/tri3b/graph_tgpr data/test_dev93 \"exp/tri4a_nnet1 exp/tri4a_nnet2 exp/tri4a_nnet3\" exp/decode_dev93_nnets_tgpr"
...
for f in $graphdir/HCLG.fst $data/feats.scp  $extra_files; do
...
t=0
for srcdir in $nnets; do
  
  [ ! -f $srcdir/$iter.mdl ] && echo "$0: no such file $f" && exit 1;
...
if [ $stage -le 1 ]; then
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    local/decodeEM/nnet-latgen-faster$thread_string \
     --minimize=$minimize --max-active=$max_active --min-active=$min_active --beam=$beam \
     --lattice-beam=$lattice_beam --acoustic-scale=$acwt --allow-partial=true --combine-function="maxone" \
     --word-symbol-table=$graphdir/words.txt \"${model[@]}\" \
     $graphdir/HCLG.fst \"${feature[@]}\" "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
fi
