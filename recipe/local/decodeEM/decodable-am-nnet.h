...
class DecodableAmNnet: public DecodableInterface {
 public:
  ...
  virtual void SetLogLikelihood(SubVector<BaseFloat> v, int32 frame) {
    log_probs_.CopyRowFromVec(v,frame);
  }

  virtual int32 NumFramesReady() const { return log_probs_.NumRows(); }

  virtual SubVector<BaseFloat> FrameLogLikelihood(int32 frame) { return log_probs_.Row(frame); }
